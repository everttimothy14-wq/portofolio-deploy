...
async function loadPublicData() {
    try {
        ...
        if (!contentType.includes('application/json')) {
            throw new Error('Backend belum berjalan. Buka website lewat http://127.0.0.1:5000/');
        }
...
function setupContactForm() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                showStatus(result.message || 'Pesan berhasil dikirim.', 'success');
                contactForm.reset();
            } else {
                showStatus(result.error || 'Gagal mengirim pesan.', 'error');
            }
        } catch (error) {
